...
/* Compared to original usbprogjtag.c, this version just adds Commands and CommandAnswer
	from main.c.  Commands is unchanged, CommandAnswer is.
*/
...
{
	// data is already in the answer buffer so just send it next time an IN packet is requested
	// but make sure you send the correct number of bytes
	answerNeedsToBeSentFlag = 1;
	sendAnswerLength = length;
}
