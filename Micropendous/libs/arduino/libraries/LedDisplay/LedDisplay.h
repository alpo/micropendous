/*
  LedDisplay -- controller library for Avago HCMS-297x displays -- version 0.1
  ...
  Uses font5x7.h by Pascal Stang, and inherits methods from the Arduino
  Print library by David A. Mellis.
...
class LedDisplay : public Print {
  public:
   ...
   LedDisplay(uint8_t _dataPin, 
	...
	void write(uint8_t b);				// write a character to the display and advance cursor
  ...
  private:
	...
	void writeCharacter(char whatCharacter, byte whatPosition);	// write a character to a buffer which will
  	...
  	int cursorPos;				// position of the cursor		
   	uint8_t dotRegister[40];    // the 320-bit dot register for the LED display
