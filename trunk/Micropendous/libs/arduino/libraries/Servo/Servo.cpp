...
#else
typedef enum { _timer3 } servoTimer_t;                     // this is the sequence for timer utilization on other controllers 
#endif
...
#define SERVO_CLASS(_timer,_channel)  (servos[SERVO_INDEX(_timer,_channel)])            // macro to access servo class by timer and channel
...
static inline void handle_interrupts(servoTimer_t timer, volatile uint16_t *TCNTn, volatile uint16_t* OCRnA)
{
  if( Channel[timer] < 0 )
    *TCNTn = 0; // channel set to -1 indicated that refresh interval completed so reset the timer 
    ...
    if( SERVO_INDEX(timer,Channel[timer]) < ServoCount && SERVO_CLASS(timer,Channel[timer]).Pin.isActive == true )  
      digitalWrite( SERVO_CLASS(timer,Channel[timer]).Pin.nbr,LOW); // pulse this channel low if activated   
  }

  Channel[timer]++;    // increment to the next channel
  if( SERVO_INDEX(timer,Channel[timer]) < ServoCount && Channel[timer] < SERVOS_PER_TIMER) {
    *OCRnA = *TCNTn + SERVO_CLASS(timer,Channel[timer]).ticks;
    if(SERVO_CLASS(timer,Channel[timer]).Pin.isActive == true)     // check if activated
      digitalWrite( SERVO_CLASS(timer,Channel[timer]).Pin.nbr,HIGH); // its an active channel so pulse it high   
  }  
  else { 
...
static boolean isTimerActive(servoTimer_t timer)
{
  ...
  for(uint8_t channel=0; channel < SERVOS_PER_TIMER; channel++) {
    if(SERVO_CLASS(timer,channel).Pin.isActive == true)
      return true;
  }
  return false;
}
